...
class firebasedb with ChangeNotifier {
  ...
  Future<void> adduser(String email, String password, String name,
      String username, var age, var gender) async {
  ...
  void addData(var data, var eklenecek) async {
    await FirebaseFirestore.instance.collection("users").doc(auth.authid()).set(
        {eklenecek: data},
        SetOptions(merge: true)).then((value) => debugPrint("add rank data"));
  }

  void addGroups(var eklenecek, var data) async {
    await FirebaseFirestore.instance
        .collection("users")
        .doc(auth.authid())
        .collection("groups")
        .doc()
        .set({eklenecek: data}, SetOptions(merge: true)).then(
            (value) => debugPrint("add grups data"));
  }
  ...
  void addLikeName(var data, var eklenecek) async {
    await FirebaseFirestore.instance
        ...
        .doc()
        .set({eklenecek: data}, SetOptions(merge: true)).then(
            (value) => debugPrint("add like name data"));
  }
  ...
  void imagelike(var eklenecek, var docid, var name) async {
    ...
    await FirebaseFirestore.instance
        ...
        .doc(docid)
        .set(users, SetOptions(merge: true))
        .then((value) => debugPrint("add rank data"));
  }
  ...
  Future<void> followers(String id) async {
    ...
    await FirebaseFirestore.instance
        ...
        .doc()
        .set(followers, SetOptions(merge: true))
        .then((value) => debugPrint("add  user data"));
  }
  ...
  Future<void> follow(String id) async {
    ...
    await FirebaseFirestore.instance
        ...
        .collection("follow")
        .doc()
        .set(follow, SetOptions(merge: true))
  ...
  void addPs(var id, var gamename, var rankname) async {
    await FirebaseFirestore.instance
        ...
        .collection("id")
        .doc()
        .set({"id": id}, SetOptions(merge: true)).then(
            (value) => debugPrint("add playersearch data"));
  }
